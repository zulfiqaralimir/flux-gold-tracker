// Vercel Serverless Function to calculate NVIDIA Market Heat + Buy/Sell Signals
// Same RSI/MACD/Pulse Speed logic as api/market-heat.js, applied to the NVIDIA.
...
export default async function handler(req, res) {
    ...
    try {
        console.log('📊 Fetching NVIDIA market data from Yahoo Finance...');

        const chartUrl = 'https://query1.finance.yahoo.com/v8/finance/chart/NVDA?interval=1d&range=6mo';
        ...
        if (!result) {
            ...
            throw new Error(message || 'No NVIDIA data available');
        }
        ...
        if (series.length < 35) {
            throw new Error('Not enough NVIDIA history to calculate indicators');
        }
        ...
        console.log(`✅ NVIDIA signals calculated: ${signalType} (Strength: ${signalStrength}/10, Heat: ${latestHeat.toFixed(1)})`);
        ...
        console.error('❌ Error calculating NVIDIA signals:', error);

        res.status(500).json({
            success: false,
            error: 'Failed to calculate NVIDIA market signals. Please try again later.'
        });
    }
}
