// Vercel Serverless Function to calculate Bitcoin Market Heat + Buy/Sell Signals
// Same RSI/MACD/Pulse Speed logic as api/market-heat.js, applied to the Bitcoin.
...
export default async function handler(req, res) {
    ...
    try {
        console.log('📊 Fetching Bitcoin market data from Yahoo Finance...');

        const chartUrl = 'https://query1.finance.yahoo.com/v8/finance/chart/BTC-USD?interval=1d&range=6mo';
        ...
        if (!result) {
            ...
            throw new Error(message || 'No Bitcoin data available');
        }
        ...
        if (series.length < 35) {
            throw new Error('Not enough Bitcoin history to calculate indicators');
        }
        ...
        console.log(`✅ Bitcoin signals calculated: ${signalType} (Strength: ${signalStrength}/10, Heat: ${latestHeat.toFixed(1)})`);

        res.status(200).json({
            success: true,
            data: {
                currentHeat: latestHeat,
                price: closeValues[lastIdx],
        ...
        console.error('❌ Error calculating Bitcoin signals:', error);

        res.status(500).json({
            success: false,
            error: 'Failed to calculate Bitcoin market signals. Please try again later.'
        });
    }
}
