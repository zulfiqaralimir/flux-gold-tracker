// Vercel Serverless Function to calculate Tesla Market Heat + Buy/Sell Signals
// Same RSI/MACD/Pulse Speed logic as api/market-heat.js, applied to the Tesla.
...
export default async function handler(req, res) {
    ...
    try {
        console.log('📊 Fetching Tesla market data from Yahoo Finance...');

        const chartUrl = 'https://query1.finance.yahoo.com/v8/finance/chart/TSLA?interval=1d&range=6mo';
        ...
        if (!result) {
            ...
            throw new Error(message || 'No Tesla data available');
        }
        ...
        if (series.length < 35) {
            throw new Error('Not enough Tesla history to calculate indicators');
        }
        ...
        console.log(`✅ Tesla signals calculated: ${signalType} (Strength: ${signalStrength}/10, Heat: ${latestHeat.toFixed(1)})`);
        ...
        console.error('❌ Error calculating Tesla signals:', error);

        res.status(500).json({
            success: false,
            error: 'Failed to calculate Tesla market signals. Please try again later.'
        });
    }
}
