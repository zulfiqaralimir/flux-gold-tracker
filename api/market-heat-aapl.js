// Vercel Serverless Function to calculate Apple Market Heat + Buy/Sell Signals
// Same RSI/MACD/Pulse Speed logic as api/market-heat.js, applied to the Apple.
...
export default async function handler(req, res) {
    ...
    try {
        console.log('📊 Fetching Apple market data from Yahoo Finance...');

        const chartUrl = 'https://query1.finance.yahoo.com/v8/finance/chart/AAPL?interval=1d&range=6mo';
        ...
        if (!result) {
            ...
            throw new Error(message || 'No Apple data available');
        }
        ...
        if (series.length < 35) {
            throw new Error('Not enough Apple history to calculate indicators');
        }
        ...
        console.log(`✅ Apple signals calculated: ${signalType} (Strength: ${signalStrength}/10, Heat: ${latestHeat.toFixed(1)})`);
        ...
        console.error('❌ Error calculating Apple signals:', error);

        res.status(500).json({
            success: false,
            error: 'Failed to calculate Apple market signals. Please try again later.'
        });
    }
}
